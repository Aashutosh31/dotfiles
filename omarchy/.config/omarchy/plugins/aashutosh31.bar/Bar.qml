...
Item {
  ...
  // A thin atmospheric layer: mostly the palette's #0B0924 background, with
  // a restrained violet lift so the wallpaper remains the visual focus.
  property color atmosphericBarTop: Qt.rgba(0.043, 0.035, 0.141, 0.64)
  property color atmosphericBarBottom: Qt.rgba(0.055, 0.040, 0.160, 0.70)
  property color atmosphericBarEdge: Qt.rgba(0.565, 0.439, 0.686, 0.16)
  property color atmosphericBarHighlight: Qt.rgba(0.992, 0.757, 0.835, 0.09)
  property int atmosphericBarRadius: 12
  property int atmosphericBarMargin: 4
  ...
  component BarPanel: PanelWindow {
    ...
    readonly property int atmosphericInset: root.transparent ? 0 : root.atmosphericBarMargin
    ...
    margins {
      top: root.barHidden && root.position === "top" ? -root.barSize : ((root.position === "top" || root.vertical) ? barWindow.atmosphericInset : 0)
      bottom: root.barHidden && root.position === "bottom" ? -root.barSize : ((root.position === "bottom" || root.vertical) ? barWindow.atmosphericInset : 0)
      left: root.barHidden && root.position === "left" ? -root.barSize : ((!root.vertical || root.position === "left") ? barWindow.atmosphericInset : 0)
      right: root.barHidden && root.position === "right" ? -root.barSize : ((!root.vertical || root.position === "right") ? barWindow.atmosphericInset : 0)
    }
    ...
    Rectangle {
      id: atmosphericBarSurface

      anchors.fill: parent
      visible: !root.transparent
      radius: Math.min(root.atmosphericBarRadius, root.vertical ? width / 2 : height / 2)
      gradient: Gradient {
        orientation: root.vertical ? Gradient.Horizontal : Gradient.Vertical

        GradientStop { position: 0.0; color: root.atmosphericBarTop }
        GradientStop { position: 1.0; color: root.atmosphericBarBottom }
      }
      border.width: 1
      border.color: root.atmosphericBarEdge
    }

    Rectangle {
      anchors.left: atmosphericBarSurface.left
      anchors.right: atmosphericBarSurface.right
      anchors.top: atmosphericBarSurface.top
      anchors.leftMargin: root.atmosphericBarRadius
      anchors.rightMargin: root.atmosphericBarRadius
      anchors.topMargin: 1
      visible: !root.transparent && !root.vertical
      height: 1
      radius: 1
      color: root.atmosphericBarHighlight
    }
  ...
  component ModuleSlot: Item {
    ...
    // Shared hover wash keeps every widget (including media) in the same
    // understated visual system without changing its own implementation.
    Rectangle {
      visible: slot.hovered && !slot.dragSource && !slot.panelOpen
      anchors.fill: parent
      ...
      gradient: Gradient {
        ...
        GradientStop { position: 0.0; color: Qt.rgba(0.565, 0.439, 0.686, 0.12) }
        GradientStop { position: 1.0; color: Qt.rgba(0.992, 0.757, 0.835, 0.08) }
      }
    }
    ...
    Rectangle {
      ...
      visible: opacity > 0
      opacity: slot.panelOpen && !slot.dragSource ? 0.58 : 0
      gradient: Gradient {
        orientation: root.vertical ? Gradient.Vertical : Gradient.Horizontal

        GradientStop { position: 0.0; color: Qt.rgba(0.663, 0.502, 0.808, 0.72) }
        GradientStop { position: 1.0; color: Qt.rgba(0.941, 0.627, 0.980, 0.64) }
      }
      radius: Math.min(width, height) / 2
